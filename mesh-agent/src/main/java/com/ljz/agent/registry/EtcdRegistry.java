...
public class EtcdRegistry implements IRegistry{
    ...
    public void register(String serviceName) throws Exception {
        ...
        String loadLevel = System.getProperty("load.level");
        if ("provider".equals(type)){
            // 如果是provider，去etcd注册服务
            try {
                int port = Integer.valueOf(System.getProperty("server.port"));
                register(serviceName, port, loadLevel);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    // 向ETCD中注册服务
    public void register(String serviceName,int port, String loadLevel) throws Exception {
        // 服务注册的key为:    /dubbomesh/com.some.package.IHelloService/192.168.100.100:2000
        String strKey = MessageFormat.format("/{0}/{1}/{2}:{3}",rootPath,serviceName,
                IpHelper.getHostIp(),String.valueOf(port));
        ByteSequence key = ByteSequence.fromString(strKey);
        ByteSequence val = ByteSequence.fromString(loadLevel);
    ...
    public List<Endpoint> find(String serviceName) throws Exception {
        ...
        for (com.coreos.jetcd.data.KeyValue kv : response.getKvs()){
            ...
            String loadLevel = kv.getValue().toStringUtf8();
            ...
            endpoints.add(new Endpoint(host, port, Integer.parseInt(loadLevel)));
        }
        return endpoints;
    }
}
